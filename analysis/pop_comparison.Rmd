...
get_df_windows <- function(df_res, df_cc, population, thresh_vals = (1:10)/10, win_size = 10){
  df_cc_sub <- df_cc %>% filter(pop == population)
  df_res_sub <- df_res %>% filter(pop == population)

  q_95_c <- quantile(df_cc_sub$re, 0.95)
  frac_sig_df <- window_frac_sig(df_res, df_cc, population, win_size = win_size)
...
df_window <- get_df_windows(df_res, df_cc, population, thresh_vals = (1:10)/10, win_size = 10)
...
```{r}
df_window_plots <- function(df_window, df_res, population, subtype, plot_cutoff = 0.1){
  #print("Hello!")
