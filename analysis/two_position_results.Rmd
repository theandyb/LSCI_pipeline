...
In addition to the relative entropy statistic at each pair of positions, let's also look at which nucleotide pairs were enriched or depleted at position pairs with higher re statistics:

```{r}
df_1_2 <- resid_df_load(base_dir, pop, subtype, 1, 2)

# Idea: make a heatmap with +1 on Y, +2 on X, colour determined by singleton residual
df_1_2 %>%
  ...
  xlab("+2 Relative Position") +
  ylab("+1 Relative Position") +
  labs(fill = "Singleton \nEnrichment")
```
...
df_m2_m1 <- resid_df_load(base_dir, pop, subtype, -2, -1)
df_m2_m1 %>%
  filter(status == "singletons") %>%
  ggplot(aes(x = p2, y = p1, fill = res)) +
  geom_tile() +
  theme_classic() +
  scale_fill_distiller(palette = "RdBu", direction = -1) +
  xlab("-1 Relative Position") +
  ylab("-2 Relative Position") +
  labs(fill = "Singleton \nEnrichment")
```
...
```{r}
df_m1_1 <- resid_df_load(base_dir, pop, subtype, -1, 1)
