...
df <- df_all_sp %>%
  ...
  arrange(offset) %>%
  mutate(p_dev = pchisq(dev, 3, lower.tail = F))

df$p_dev %>% density() %>% plot()

df$q_vals <- qvalue(df$p_dev)$qvalues

ggplot(df, aes(x = p_dev, y = q_vals)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0)

df %>%
  mutate(sig_type = ifelse(q_vals < 0.05, "both_sig", 
                           ifelse(p_dev < 0.05, "p_sig", "in_sig"))) %>%
  ggplot(aes(x = offset, y = log10(re), colour = sig_type)) +
  geom_point()
```
...
df_all_sp %>%
  ...
  ggtitle(subtype_print_names("AT_GC"))

df_all_sp %>%
  filter(subtype == "AT_GC", abs(offset) > 250) %>%
  ggplot(aes(x = pop, y = log10(dev))) +
  geom_boxplot(notch = TRUE) +
  geom_hline(yintercept = log10(qchisq(0.95, 3)), colour = "red") +
  ggtitle(subtype_print_names("AT_GC"), "Exclude interior +/- 250bp")
...
## Plot all single position results
...
single_pos_stat_plot <- function(st, sp){
  n_s <- df_all_sp %>% filter(subtype == st, pop == sp) %>% pull(singletons) %>% max()
  n_c <- df_all_sp %>% filter(subtype == st, pop == sp) %>% pull(controls) %>% max()

  crit_val <- qchisq(0.95, 3) / (2 * (n_c + n_s))

  p <- df_all_sp %>%
    filter(subtype == st, pop == sp) %>%
    ggplot(aes(x = offset, y = re, colour = is_sig)) +
    geom_point() +
    geom_hline(yintercept = crit_val, linetype="dotted") +
    scale_y_log10() +
    ggtitle(sp, subtype_print_names(st))
 return(p)
}

plot_list[[sp]] <- single_pos_stat_plot(st, sp)
print(plot_list[[sp]] )
```
...
## 500 bp center windows

Here we'll evaluate the consequences of using a set +/- 250bp window for all subtypes across all populations.

```{r}
results <- data.frame(subtype = character(),
                      pop = character(),
                      true_pos = numeric(),
                      false_neg = numeric(),
                      true_neg = numeric(),
...
for(sp in c("ALL", "AFR", "AMR", "EAS", "EUR", "SAS")){
  for(st in subtypes){
    df <- df_all_sp %>%
      filter(subtype == st, pop == sp) %>%
      mutate(in_win = abs(offset) <= 250)
    true_pos <- df %>% filter(in_win, is_sig) %>% pull(is_sig) %>% length() 
    false_pos <- df %>% filter(in_win, !is_sig) %>% pull(is_sig) %>% length() 
    true_neg <- df %>% filter(!in_win, !is_sig) %>% pull(is_sig) %>% length() 
    false_neg <- df %>% filter(!in_win, is_sig) %>% pull(is_sig) %>% length()
    
    prop_int_sig <- {df %>% filter(in_win, is_sig) %>% pull(is_sig) %>% length()} / {df %>% filter(in_win) %>% pull(is_sig) %>% length()}
    prop_ext_sig <- {df %>% filter(!in_win, is_sig) %>% pull(is_sig) %>% length()} / {{df %>% filter(!in_win) %>% pull(is_sig) %>% length()}}
    
    t_sig <- df %>% pull(is_sig) %>% sum()
    t_f <- df %>% filter(!is_sig) %>% pull(is_sig) %>% length()
    ...
    results <- bind_rows(results,
                         data.frame(subtype = st,
                                    ...
                                    t_sig = t_sig,
                                    t_f = t_f, 
                                    prop_int_sig = prop_int_sig,
                                    prop_ext_sig = prop_ext_sig))
  }
}

results$precision <- results$true_pos / (results$true_pos + results$false_pos)
results$recall <- results$true_pos / (results$true_pos + results$false_neg)
```

```{r}
results %>%
  select(subtype, pop, precision, recall) %>%
  pivot_longer(precision:recall, names_to = "measure", values_to = "value") %>%
  ggplot(aes(x = subtype, y = value, colour = measure)) +
  geom_point() +
  facet_grid(rows = vars(pop))

results %>%
  select(subtype, pop, prop_ext_sig) %>%
  ggplot(aes(x = subtype, y = prop_ext_sig, colour = pop)) +
  geom_point()

results %>%
  ggplot(aes(x = subtype, y = precision, colour = pop)) +
  geom_point() +
  theme_classic() +
  xlab("Subtype") +
  ylab("Precision") +
  scale_color_manual(values = ggpubfigs::friendly_pals$vibrant_seven)

results %>%
  mutate(fnr = false_neg / t_sig) %>%
  ggplot(aes(x = subtype, y = fnr, colour = pop)) +
  geom_point() +
  theme_classic() +
  xlab("Subtype") +
...
## Find windows by alpha

Let's expand the window by 10 bp intervals, and in each we'll compute the fraction of significant results outside of the window. We'll see what the smallest window for each subtype is in which the fraction of significant observations is less than, say, 5 or 10%.
...
```{r}
subpop <- "ALL"
results <- data.frame(window = numeric(),
                      frac_ext_sig = numeric(),
                      frac_int_sig = numeric())
df <- df_all_sp %>%
  filter(subtype == st, pop == subpop)

for(i in seq(10,900, by = 10)){
  df <- df %>%
    mutate(in_win = abs(offset) <= i)
  
  frac_ext_sig <- length({df %>% filter(is_sig, !in_win) %>% pull(is_sig)}) / length({df %>% filter(!in_win) %>% pull(is_sig)})
  frac_int_sig <- length({df %>% filter(is_sig, in_win) %>% pull(is_sig)}) / length({df %>% filter(in_win) %>% pull(is_sig)})
  results <- bind_rows(results, data.frame(window = i,
                                          frac_ext_sig = frac_ext_sig,
                                          frac_int_sig = frac_int_sig))
}
```

```{r}
results %>%
  pivot_longer(-window, names_to = "location", values_to = "prop") %>%
  ggplot(aes(x = window, y= prop, colour = location)) +
  geom_point() +
  geom_line()
```
...
window_finder_diff <- function(df_all_sp, st, p, alpha = 0.05){
...
list_res <- window_finder_diff(df_all_sp, "AT_CG", "AMR")
```
...
## Use FP/FN Intersection as Window Selction Criterion
...
for(i in subtypes){
  for(j in c("ALL", "AFR", "AMR", "EAS", "EUR", "SAS")){
    print(paste0(i, " - ", j))
    res_item <- window_finder_diff(df_all_sp, i, j)
...
plot_rate_list[[st]][["ALL"]]
plot_rate_list[[st]][["AFR"]]
plot_rate_list[[st]][["AMR"]]
plot_rate_list[[st]][["EAS"]]
plot_rate_list[[st]][["EUR"]]
plot_rate_list[[st]][["SAS"]]
```
...
```{r}
df_resid <- vector(mode = "list", length = 9)
names(df_resid) <- subtypes
...
for(st in subtypes){
  ...
  df_resid[[st]] <- sp_resid_all_pop(single_pos_dir, st, rp = pos_vec)
  df_resid[[st]]$subtype <- st
}

df_resid <- bind_rows(df_resid)
```

```{r}
df_resid %>%
  filter(status == "singletons", subtype == "AT_GC", pop != "ALL") %>%
  select(rp, nuc, pop, res) %>%
  pivot_wider(id_cols = c("rp", "nuc"), names_from = "pop", values_from = "res") %>%
  group_by(rp) %>%
  group_modify(~ correlate(.x)) %>% summary()
```
...
In the A to C subtype, we observe the strongest signal of influence at the -1 relative position. 

```{r}
df_resid %>%
  filter(status == "singletons", subtype == "AT_CG", pop != "ALL", rp == -1) %>%
  mutate(re_res = re_res * sign(res)) %>%
  select(rp, nuc, pop, re_res) %>%
  pivot_wider(id_cols = c("rp", "nuc"), names_from = "pop", values_from = "re_res")
```
...
In the A > G subtype, the strongest signal is observed at the +1 relative position across all populations. In the singletons, we see the following patterns of nucleotide enrichment/depletion across the populations:

```{r}
df_resid %>%
  filter(status == "singletons", subtype == "AT_GC", pop != "ALL", rp == 1) %>%
  mutate(re_res = re_res * sign(res)) %>%
  select(rp, nuc, pop, re_res) %>%
  pivot_wider(id_cols = c("rp", "nuc"), names_from = "pop", values_from = "re_res")
```

Here we observe largely consistent results in regards to the rank of nucleotide by relative entropy residuals and sign, with the exception of C in AMR; the C nucleotide has the smallest absolute value across all populations, however, and the results at the more influential nucleotides (i.e. T and A) are consistent across all populations including AMR.
...
In the A to T subtype, we observe the strongest signal of influence at the +1 relative position. 

```{r}
df_resid %>%
  filter(status == "singletons", subtype == "AT_TA", pop != "ALL", rp == 1) %>%
  mutate(re_res = re_res * sign(res)) %>%
  select(rp, nuc, pop, re_res) %>%
  pivot_wider(id_cols = c("rp", "nuc"), names_from = "pop", values_from = "re_res")
```
...
In the C to T subtype, we observe the strongest signal of influence at the -1 relative position. 

```{r}
df_resid %>%
  filter(status == "singletons", subtype == "GC_AT", pop != "ALL", rp == -1) %>%
  mutate(re_res = re_res * sign(res)) %>%
  select(rp, nuc, pop, re_res) %>%
  pivot_wider(id_cols = c("rp", "nuc"), names_from = "pop", values_from = "re_res")
```
...
In the C to A subtype, we observe the strongest signal of influence at the +1 relative position. 

```{r}
df_resid %>%
  filter(status == "singletons", subtype == "GC_TA", pop != "ALL", rp == 1) %>%
  mutate(re_res = re_res * sign(res)) %>%
  select(rp, nuc, pop, re_res) %>%
  pivot_wider(id_cols = c("rp", "nuc"), names_from = "pop", values_from = "re_res")
```
...
In the C to G subtype, we observe the strongest signal of influence at the -2 relative position. 

```{r}
df_resid %>%
  filter(status == "singletons", subtype == "GC_CG", pop != "ALL", rp == -2) %>%
  mutate(re_res = re_res * sign(res)) %>%
  select(rp, nuc, pop, re_res) %>%
  pivot_wider(id_cols = c("rp", "nuc"), names_from = "pop", values_from = "re_res")
```
...
In the CpG to TpG subtype, we observe the strongest signal of influence at the -1 relative position. 

```{r}
df_resid %>%
  filter(status == "singletons", subtype == "cpg_GC_AT", pop != "ALL", rp == -1) %>%
  mutate(re_res = re_res * sign(res)) %>%
  select(rp, nuc, pop, re_res) %>%
  pivot_wider(id_cols = c("rp", "nuc"), names_from = "pop", values_from = "re_res")
```
...
In the CpG to ApG subtype, we observe the strongest signal of influence at the -1 relative position. 

```{r}
df_resid %>%
  filter(status == "singletons", subtype == "cpg_GC_TA", pop != "ALL", rp == -1) %>%
  mutate(re_res = re_res * sign(res)) %>%
  select(rp, nuc, pop, re_res) %>%
  pivot_wider(id_cols = c("rp", "nuc"), names_from = "pop", values_from = "re_res")
```
...
In the CpG to GpG subtype, we observe the strongest signal of influence at the -1 relative position for all but the AFR population. 

```{r}
df_resid %>%
  filter(status == "singletons", subtype == "cpg_GC_CG", pop != "ALL", rp == -1) %>%
  mutate(re_res = re_res * sign(res)) %>%
  select(rp, nuc, pop, re_res) %>%
  pivot_wider(id_cols = c("rp", "nuc"), names_from = "pop", values_from = "re_res")
```
