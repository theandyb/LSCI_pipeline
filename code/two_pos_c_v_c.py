"""
Compare distributions of nucleotides at two flanking positions between
samples of control observations to derive an empirical null
"""
import pandas as pd
...
def c_pos(subtype, chromosome, pop, start = 0, skip = 5):
...
@ray.remote
def get_3mer_control(chromosome, pop, subtype, rel_vals = [-1, 0, 1], start = 0, skip = 5):
  motif_size = len(rel_vals)
  control_pos = c_pos(subtype, chromosome, pop, start = start, skip = skip)
  rev_control_pos = c_pos(subtype + "_rev", chromosome, pop, start = start, skip = skip)
...
def get_count_all(subtype, pop="ALL", rel_vals =[-1,1], start = 0, skip = 5):
  futures = [get_3mer_control.remote(i, pop, subtype, rel_vals = rel_vals, start = start, skip = skip) for i in range(1,23)]
  results = pd.DataFrame.from_dict(ray.get(futures)).sum(axis = 0)
  return(results)

def fit_model_all(subtype, p, q, pop = "ALL", alt_start = 1, alt_skip = 5):
  df_s = get_count_all(subtype, pop = pop, rel_vals=[p, q])
  df_c = get_count_all(subtype, pop = pop, rel_vals=[p, q], start = alt_start, skip = alt_skip)
...
out_dir = "output/two_pos_cc/resid/{}/".format(pop)
