...
```{r}
df_all %>%
  filter(subtype %in% c("AT_GC", "all_GC_AT"), abs(offset) <= 20) %>%
  select(offset, subtype, cramer) %>%
  ggplot(aes(x = offset, y = cramer, colour = subtype)) +
  geom_point() +
  theme_classic()
```
...
### Counts of Singletons

#### ALL

```{r}
df_all %>%
  mutate(subtype = subtype_print_names(subtype)) %>%
  group_by(subtype) %>%
  summarize(singletons = max(singletons)) %>%
  knitr::kable()
```
...
```{r}
load_all_pos_results(subtypes, pop = "AFR") %>%
  mutate(subtype = subtype_print_names(subtype)) %>%
  group_by(subtype) %>%
  summarize(singletons = max(singletons)) %>%
  knitr::kable()
...
```{r}
load_all_pos_results(subtypes, pop = "AMR") %>%
  mutate(subtype = subtype_print_names(subtype)) %>%
  group_by(subtype) %>%
  summarize(singletons = max(singletons)) %>%
  knitr::kable()
...
```{r}
load_all_pos_results(subtypes, pop = "EAS") %>%
  mutate(subtype = subtype_print_names(subtype)) %>%
  group_by(subtype) %>%
  summarize(singletons = max(singletons)) %>%
  knitr::kable()
...
```{r}
load_all_pos_results(subtypes, pop = "EUR") %>%
  mutate(subtype = subtype_print_names(subtype)) %>%
  group_by(subtype) %>%
  summarize(singletons = max(singletons)) %>%
  knitr::kable()
...
```{r}
load_all_pos_results(subtypes, pop = "SAS") %>%
  mutate(subtype = subtype_print_names(subtype)) %>%
  group_by(subtype) %>%
  summarize(singletons = max(singletons)) %>%
  knitr::kable()
...
## Temp: Working on text of draft

### What are the big deals?

1. We've utilized a dataset comprising of evolutionarily newer variants (singletons)

### Text

Our analyses investigated the influence of neighboring nucleotides on germline mutation rates. We assessed the influence of nucleotides at individual flanking positions in an expanded window covering flanking positions up to 1000 bp from the focal position. Second and third order interactions were assessed across all combinations of flanking positions within +/- 10 bp of the focal position to ascertain the degree to which neighboring nucleotides interact and moderate each other’s influence on the rate of mutation. We conducted these analyses using singletons aggregated across all 1000 Genomes Project (1kGP) populations and separately for each of the five 1kGP super-populations. We then assessed the consistency of population-level results for both single-position and higher-order models.

#### Patterns of influence at individual flanking positions in an expanded window

We first examined the influence of nucleotides at individual flanking positions within a small window (e.g., +/- 10bp from the focal position), consistent with previous research on local sequence context influence on germline mutation rates (citation). However we considered nine distinct mutation categories, as we addressed the known impact of CpG dinucleotides on mutation rates by separately evaluating CpG and non-CpG mutations as distinct categories. Across all nine mutation types, we observed highly statistically significant deviance statistics at all flanking positions within 10 bases of the focal position, ranging from 18.6 to 1659567 ($\chi^2$ df = 3, $p \leq 3.24 \times 10^-4$). 

```{r}
df_all %>%
  filter(abs(offset) <= 10) %>%
  group_by(subtype) %>%
  summarize(mean_d = mean(dev),
            min_d = min(dev),
            max_d = max(dev),
            singletons = max(singletons)) %>%
  mutate(min_p = pchisq(min_d, df = 3, lower.tail = F),
         max_p = pchisq(max_d, df = 3, lower.tail = F)) %>%
  ungroup() %>%
  rowwise() %>%
  mutate(subtype = subtype_print_names(subtype)) %>%
...
df_tmp <- df_all %>%
  ...
  select(subtype, offset, dev, re) %>%
  mutate(p = pchisq(dev, df = 3, lower.tail = F)) %>%
  arrange(subtype, offset)

df_tmp[which(str_starts(df_tmp$subtype, "GC") & df_tmp$offset == 1),]$p <- pchisq(df_tmp[which(str_starts(df_tmp$subtype, "GC") & df_tmp$offset == 1),]$dev, df = 3, lower.tail = F)
...
df_all %>%
  ...
  summarize(mean_d = mean(dev),
            min_d = min(dev),
            max_d = max(dev),
            singletons = mean(singletons)) %>%
  ggplot(aes(x = singletons, y = mean_d)) +
  geom_point()
...
df_all %>%
  ...
  summarize(mean_d = mean(dev),
            ...
            max_d = max(dev),
            singletons = mean(singletons)) %>%
  {cor.test(.$max_d, .$singletons)}
```

### Practical implications of deviance statistics

We have a distribution of deviance statistics, but what practical significance do they have? (as opposed to statistically significant?)

### Deviance statistics in broader window

Although the relative entropy statistics decrease as positions further from the focal position are considered, all deviance statistics in the +/- 10 bp window are highly significant (p<7e-6) in all mutation subtypes.

```{r}
df_all %>%
  filter(subtype == "AT_GC") %>%
  mutate(is_sig = pchisq(dev, 3, lower.tail = F) < 0.05) %>%
...
p1 <- df_all %>%
  ...
  ggplot(aes(x = offset, y = re, group = side)) +
  geom_point() +
  geom_line() +
  theme_bw() +
...
#### Combo Resid + Statistic

```{r}
resid_plot(single_pos_dir, "ALL", "AT_GC", rp = c(-5:-1,1:5))
...
resid_combo_plot <- function(res_dir, pop, st, df_all, rp = c(-5:-1,1:5)){
  ...
  p <- res_df2 %>%
    ...
    scale_x_discrete(labels = xlab, drop = FALSE) +
    geom_bar(position = "stack", stat = "identity") +
    geom_point(aes(y = re, fill = NULL), data = df_stat, show.legend = F, size = 2) +
    ...
    ylab("Relative Entropy") +
    theme_bw() +
    xlab(paste0("Relative Position: ", subtype_print_names(st))) +
    labs(fill = "") +
...
updated_plot_list[["cpg_GC_AT"]]
```

```{r fig.height = 8, fig.width = 8, fig.align = "center"}
sjPlot::plot_grid(x = updated_plot_list, margin = c(0.3, 0.3, 0.3, 0.3))
```
...
for(st in subtypes){
  plot_list[[st]] <- resid_logo_plot(single_pos_dir, "ALL", st, df_all, rp = c(-5:-1,1:5))
}
...
df_all %>%
  filter(abs(offset) <= 10) %>%
  select(dev, re, mean_var, max_var, simple_re) %>%
  summary() %>%
  knitr::kable()
```
