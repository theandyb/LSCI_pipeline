...
def get_count_all(subtype, offset, pop, base_dir,  status = "singleton"):
  ...
  """
  if status == "singleton":
    futures = [get_count_table_control.remote(i, subtype, offset, pop, base_dir, ".min") for i in range(1,23)]
    results = pd.DataFrame.from_dict(ray.get(futures)).sum(axis=0)
  else:
    futures = [get_count_table_control.remote(i, subtype, offset, pop, base_dir, ".max") for i in range(1,23)]
